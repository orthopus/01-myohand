...
private:
  ...
  // Motor Command
  CommandSignal motor_cmd = CommandSignal();
  CommandSignal control_cmd = CommandSignal();
  CommandSignal last_stall_cmd = CommandSignal();
...
public:
  ...
  Status stall_handler(const CommandSignal &emg_cmd, const Sensor &current,
                       const Sensor &battery);

  // Set control_cmd to zero
  void stop();
  ...
  CommandSignal getMotorCommand() const;
